...
export const LIGHTNING_LIFETIME=2.5;
const SEGMENTS=160,axis=new T.Vector3(0,1,0),cylinder=new T.CylinderGeometry(1,1,1,12,1,true);
const layers=[{color:0x0862ff,radius:11,opacity:.34},{color:0x16baff,radius:4.5,opacity:.6},{color:0xddffff,radius:1,opacity:.95}];
...
#include <colorspace_fragment>
  ...
  update(age){
    ...
    this.meshes.forEach((mesh,i)=>mesh.material.uniforms.alpha.value=layers[i].opacity*fade*(.86+.14*Math.sin(age*45)));
    ...
    for(let b=0;b<10;b++){
      ...
      for(let k=1;k<=8;k++){const f=k/8;const point=start.clone().addScaledVector(this.direction,(b%2?1:-.25)*f*length*.22).addScaledVector(this.side,Math.cos(angle)*f*spread+wave(k+b,1)*.1*f).addScaledVector(this.up,Math.sin(angle)*f*spread+wave(k+b+3,.8)*.12*f);segments.push([prev,point,.017*(1-f*.65)]);if(k===5){const tip=point.clone().addScaledVector(this.side,Math.sin(angle)*.35).addScaledVector(this.up,Math.cos(angle)*.35);segments.push([point,tip,.007])}prev=point}
    }
