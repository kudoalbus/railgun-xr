...
import {coin,frontSVG,backSVG} from './coin.js';
...
const ring=new T.Mesh(new T.TorusGeometry(.63,.0015,6,100),new T.MeshBasicMaterial({color:0x2b6985}));ring.position.copy(preview.position);scene.add(ring);
...
state.held.position.copy(thumb).addScaledVector(side,.014);
...
// Export editable vector masters from the preview with a single click.
for(const [name,svg]of [['正面',frontSVG],['反面',backSVG]]){const a=document.createElement('a');a.textContent=`${name} SVG`;a.href=URL.createObjectURL(new Blob([svg],{type:'image/svg+xml'}));a.download=`coin-${name}.svg`;a.style.cssText='font-size:10px;color:#779dad;margin-left:12px';$('footer').append(a)}
